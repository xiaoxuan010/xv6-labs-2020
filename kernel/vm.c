...
void u2kvmcopy(pagetable_t pgtbl, pagetable_t kpgtbl, uint64 oldSize, uint64 newSize)
{
  ...
  for (addr = oldSize; addr < newSize; addr += PGSIZE)
  {
    if ((pte_src = walk(pgtbl, addr, 0)) == 0)
    {
      panic("u2kvmcopy: pte should exist");
    }
    if ((pte_dst = walk(kpgtbl, addr, 1)) == 0)
    {
      panic("u2kvmcopy: pte should exist");
    }
    paddr = PTE2PA(*pte_src);
    flags = PTE_FLAGS(*pte_src) & (~PTE_U);
    *pte_dst = PA2PTE(paddr) | flags;
  }
}
...
void vmprint_rec(pagetable_t pgtbl, int level)
{
  for (int i = 0; i < 512; i++)
  {
    pte_t pte = pgtbl[i];
